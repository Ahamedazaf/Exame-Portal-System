...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (user) { redirected.current = true; router.replace(user.role === 'teacher' ? '/teacher/dashboard' : '/student/dashboard'); }
  ...
  const handleGoogle = async () => { setGoogleLoading(true); setError(''); await signIn('google', { callbackUrl: '/login' }); };

  // Pending approval
  if (!loading && pendingApproval) return (
    <div className="min-h-screen flex items-center justify-center p-4" style={{background:'linear-gradient(135deg,#2e1065 0%,#4c1d95 50%,#6d28d9 100%)'}}>
      <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl p-8 text-center animate-fade-in">
        <div className="w-20 h-20 bg-amber-50 rounded-full flex items-center justify-center mx-auto mb-5"><Clock size={36} className="text-amber-500" /></div>
        <h2 className="text-2xl font-extrabold text-slate-800 mb-2">Approval Pending</h2>
        <p className="text-slate-500 text-sm mb-6">Hi <strong>{pendingUser?.name}</strong>! The teacher needs to approve your account.</p>
        ...
        <button onClick={logout} className="w-full btn-secondary py-3 text-sm">Sign Out / Use Different Account</button>
      </div>
    </div>
  );
      ...
      <div className="hidden lg:flex flex-col justify-between w-5/12 xl:w-1/2 p-12 xl:p-16 text-white">
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 rounded-2xl flex items-center justify-center overflow-hidden" style={{background:'rgba(255,255,255,0.15)'}}>
            {portalLogoUrl ? <img src={portalLogoUrl} alt="logo" className="w-full h-full object-cover" />
              : <span className="text-white font-extrabold">{portalLogoText}</span>}
          </div>
          <span className="text-xl font-extrabold">{portalName}</span>
        </div>
        <div>
          <div className="inline-block bg-white/10 rounded-2xl px-4 py-2 text-sm font-semibold text-purple-200 mb-6">🎓 Online Examination System</div>
          ...
          <p className="text-purple-200 text-base leading-relaxed max-w-sm mb-8">
            Take class-based MCQ exams, get instant results, and track your progress — all in one place.
          </p>
          ...
          <div className="bg-white rounded-3xl shadow-2xl overflow-hidden animate-fade-in">
            ...
            <div className="p-6 sm:p-8 lg:p-10">
              ...
              <button onClick={handleGoogle} disabled={googleLoading || submitting}
                ...
                {googleLoading ? <span className="w-5 h-5 border-2 border-slate-300 border-t-purple-600 rounded-full animate-spin" />
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <input type="email" required autoComplete="email" value={form.email}
                    onChange={e=>setForm({...form,email:e.target.value})} placeholder="you@example.com" className="input-field text-base" />
                </div>
                <div>
              ...
              <p className="mt-5 text-center text-sm text-slate-500">
                New student? <Link href="/register" className="font-bold hover:underline" style={{color:'#7c3aed'}}>Create an account</Link>
              </p>
            </div>
          </div>
        </div>
